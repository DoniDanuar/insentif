...
class Mapping_insentif extends CI_Controller {

    var $controller     = 'Mapping_insentif'; 
    var $loadViewList   = 'mapping_insentif/list';
    var $loadViewForm   = 'mapping_insentif/form';

    var $formNameHead   = 'Mapping_insentif';
    var $formNameData   = 'Data Mapping_insentif';
    var $formNameAdd    = 'Form Tambah Data';
    var $formNameEdit   = 'Form Edit Data';

    var $menu           = 'Mapping_insentif';

    public function __construct()
    {
        parent::__construct();
        $this->isLogin();
        $this->load->model('Mapping_insentif_model');
    ...
    public function edit($primaryKey)
    {
        ...
        if ($this->Mapping_insentif_model->getById($primaryKey)->num_rows() == 0) {
            $pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
    ...
    public function hapus($primaryKey)
    {
        ...
        $hapus = $this->Mapping_insentif_model->delete($primaryKey);
        if ($hapus) {
    ...
    public function simpan()
    {
        ...
        $tglmapping = $this->input->post('tglmapping');
        $idkaryawan = $this->input->post('idkaryawan');
        $idinsentif = $this->input->post('idinsentif');
        $besarbonus = $this->input->post('besarbonus');
        $tagerterjual = $this->input->post('tagerterjual');
        ...
        if ($idmapping == '') {
            ...
            $data = array(
                'idmapping' => $idmapping, 
                'tglmapping' => $tglmapping,
                'idkaryawan' => $idkaryawan,
                'idinsentif' => $idinsentif,
                'besarbonus' => $besarbonus,
                'tagerterjual' => $tagerterjual,
            );

            $simpan = $this->Mapping_insentif_model->insert($data);
            if ($simpan) {
            ...
            $simpan = $this->Mapping_insentif_model->update($data, $idmapping);
            if ($simpan) {
    ...
    public function getEditData()
    {
        ...
        $result                 = $this->Mapping_insentif_model->getById($primaryKey)->row();

        $data = array(
            'idmapping' => $result->idmapping, 
            'tglmapping' => $result->tglmapping, 
            'idkaryawan' => $result->idkaryawan, 
            'idinsentif' => $result->idinsentif, 
            'besarbonus' => $result->besarbonus, 
            'tagerterjual' => $result->tagerterjual, 
        );
    ...
    function getData()
    {
        $data       = $this->Mapping_insentif_model->get_datatables();
        $dataArr    = array();
        $no         = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Mapping_insentif_model->count_all(),
            "recordsFiltered" => $this->Mapping_insentif_model->count_filtered(),
            "data" => $dataArr,
        );
...
/* End of file Mapping_insentif.php */
/* Location: ./application/controllers/Mapping_insentif.php */