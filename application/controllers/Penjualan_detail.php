...
class Penjualan_detail extends CI_Controller {
    ...
    var $controller     = 'Penjualan_detail'; 
    var $loadViewList   = 'penjualan_detail/list';
    var $loadViewForm   = 'penjualan_detail/form';

    var $formNameHead   = 'Penjualan_detail';
    var $formNameData   = 'Data Penjualan_detail';
    var $formNameAdd    = 'Form Tambah Data';
    var $formNameEdit   = 'Form Edit Data';

    var $menu           = 'Penjualan_detail';
    ...
    public function edit($primaryKey)
    {
        ...
        if ($this->Penjualan_detail_model->getById($primaryKey)->num_rows() == 0) {
            $pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
    ...
    public function hapus($primaryKey)
    {
        ...
        $hapus = $this->Penjualan_detail_model->delete($primaryKey);
        if ($hapus) {
    ...
    public function simpan()
    {
        ...
        if ($ == '') {
            ...
            $ = $this->db->query("SELECT f__create() AS ")->row()->;
            ...
            $simpan = $this->Penjualan_detail_model->insert($data);
            if ($simpan) {
            ...
            $simpan = $this->Penjualan_detail_model->update($data, $);
            if ($simpan) {
    ...
    public function getEditData()
    {
        ...
        $result                 = $this->Penjualan_detail_model->getById($primaryKey)->row();
    ...
    function getData()
    {
        $data       = $this->Penjualan_detail_model->get_datatables();
        $dataArr    = array();
        $no         = $_POST['start'];
        ...
        foreach ($data as $row) {
            ...
            $arr[]  = '
                        <a href="'.site_url( $this->controller.'/edit/'.$this->encrypt->encode($row->) ).'" class="btn btn-sm btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>  
                     
                        <a href="'.site_url( $this->controller.'/hapus/'.$this->encrypt->encode($row->) ).'" class="btn btn-sm btn-danger btn-circle" id="hapus" title="Hapus Data">
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Penjualan_detail_model->count_all(),
            "recordsFiltered" => $this->Penjualan_detail_model->count_filtered(),
            "data" => $dataArr,
        );
